...
@onready var wheel : MeshInstance3D = $CollisionPoint
...
var last_point = wheel.global_position

func _ready():
	raycast.target_position.y = -car_p.sus_height
...
func _physics_process(delta):
	...
	if(is_on_ground()):
		...
		displacement = (raycast.target_position - (raycast.get_collision_point()) * global_transform).length()
...
func calc_vel_vector():
	wheel_vel_vector = global_position - last_point
